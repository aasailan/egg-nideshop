 ...
 * @Last Modified time: 2018-07-16 20:46:19
 * 购物车表
 */
...
export default (app: Application) => {
   ...
   */
  cart.addGoods = (userId: number, goodsId: number, productId: number, number: number,
    goodsInfo: IGoodInst) => app.model.transaction(async t => {
      ...
      // NOTE: 减少该规格商品货物数 应该等到下单(checkout)的时候才减少货物数
      // await app.model.Product.update({
      //   goods_number: productInfo.goods_number - number,
      // }, {
      //   where: { goods_id: goodsId, id: productId },
      //   transaction: t,
      // });
  ...
  cart.updateGoods = (goodsId: number, productId: number, number: number,
    cartId: number) => app.model.transaction(async t => {
      // 查询货物规格信息，判断是否有足够的存货
      const productInfo = await app.model.Product.find({
        where: { goods_id: goodsId, id: productId },
        raw: true,
